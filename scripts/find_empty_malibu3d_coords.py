"""
Scan Malibu3D scenes listed in the manifest and detect invalid coord files.

The script reads scene definitions from a CSV manifest (same logic as
Malibu3DDataset), skips HARDCODED_EXCLUDED_TILES, and reports scenes where:
- coord.npy is empty
- coord.npy is missing (optional)
- coord.npy cannot be loaded

Primary output format is compatible with Malibu3DDataset.missing_tiles_manifest:
  split,patch_id

Example:
  python scripts/find_empty_malibu3d_coords.py \
...
MALIBU3D_DATASET_FILE = os.path.join(REPO_ROOT, "pointcept", "datasets", "malibu3d.py")
...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Find Malibu3D scenes with empty coord.npy.")
    parser.add_argument(
        "--data_root",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default=os.path.join("data", "malibu3d_plus", "missing_coord_tiles.txt"),
...
def load_hardcoded_excluded_tiles() -> set[tuple[str, str]]:
    """
    Read HARDCODED_EXCLUDED_TILES from pointcept/datasets/malibu3d.py using AST.

    This avoids importing the full project package and its optional dependencies.
    """
    with open(MALIBU3D_DATASET_FILE, "r", encoding="utf-8") as handle:
        module_ast = ast.parse(handle.read(), filename=MALIBU3D_DATASET_FILE)

    for node in module_ast.body:
        if not isinstance(node, ast.ClassDef) or node.name != "Malibu3DDataset":
            continue
    ...
    raise RuntimeError(
        "Could not locate Malibu3DDataset.HARDCODED_EXCLUDED_TILES in malibu3d.py."
    )
...
def main() -> None:
    ...
    print("You can set this file as missing_tiles_manifest in your Malibu3D config.")
