...
# Keep in sync with scripts/convert_flair3d_splits_to_json.py (INPUT_BY_SPLIT).
...
# Paths relative to dataset_root (Flair3D Plus raw directory).
...
def build_modality_patch_path(
    ...
    """Same logic as pointcept...preprocess_flair3d.build_modality_patch_path (no package import)."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--dataset_root",
        type=str,
        default=os.path.join("data", "flair3d_plus", "raw"),
        help=(
            "Flair3D Plus raw directory (relative to repo root if not absolute). "
