#!/usr/bin/env python3
"""Visualize Flair3D network GT masks, optional pred probs/binary/graph, and mean RGB.
...
_FLAIR3D_PLUS_DIR = os.path.join(
    REPO_ROOT, "pointcept", "datasets", "preprocessing", "flair3d_plus"
)
if _FLAIR3D_PLUS_DIR not in sys.path:
    ...
    sys.path.insert(0, _FLAIR3D_PLUS_DIR)
...
def main() -> None:
    ...
    mode.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a preprocessed Flair3D+ subtile directory "
    ...
    mode.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a Flair3D+ ROI directory containing preprocessed subtiles. "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional Flair3D-build exported-graphs root "
        "(e.g. /data/geist/Flair3D-build/data/network_graphs). If set, shows the "
