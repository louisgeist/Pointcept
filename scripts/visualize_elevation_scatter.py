...
3. no args -- both D075 test zones used in README_elevation_parity_geist.md,
   from the cached ``pairs.npz`` if present, else from the 873542 dumps.
...
Example::
    ...
    python scripts/visualize_elevation_scatter.py \\
      --pairs stats/flair3d/elevation_parity/D075-2021_AA-S2-2/pairs.npz
...
DEFAULT_GT_ROOT = REPO_ROOT / "data" / "flair3d_plus" / "test" / "D075-2021_LIDARHD"
DEFAULT_PARITY_DIR = REPO_ROOT / "stats" / "flair3d" / "elevation_parity"
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="PNG path (single plot) or directory (several). Default: stats/flair3d/elevation_parity/<zone>/hexbin.png",
    )
