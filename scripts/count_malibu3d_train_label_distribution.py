#!/usr/bin/env python3
"""
Compute per-class label distributions on Malibu3D+ train (or other splits).

Uses the same scene list as Malibu3DDataset (CSV manifest, LIDARHD=True, excluded tiles).
...
(MALIBU3D_TILE_DISTRIBUTION_TASKS), fan-out remapped from natural_habitat.npy with the
same LUTs as Malibu3DLabelRemap in training (storage natural_habitat=default required).
...
LUT logic as Malibu3DLabelRemap. Axis histograms always use storage natural_habitat and the
...
--missing_tiles_manifest data/malibu3d_plus/missing_ply_preflight.txt \
--too_small_tiles_manifest data/malibu3d_plus/too_small_tiles.csv \
--storage_definitions natural_habitat=default \
--num_workers 24 \
--output_dir stats/malibu3d/label_distribution
"""
...
def _load_malibu3d_config_utils():
    path = os.path.join(REPO_ROOT, "pointcept", "datasets", "malibu3d_config_utils.py")
    spec = importlib.util.spec_from_file_location("malibu3d_config_utils", path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load malibu3d_config_utils from {path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


_malibu3d_cfg = _load_malibu3d_config_utils()
MALIBU3D_SEMANTIC_TARGET_KEYS = _malibu3d_cfg.MALIBU3D_SEMANTIC_TARGET_KEYS
MALIBU3D_TILE_DISTRIBUTION_TASKS = _malibu3d_cfg.MALIBU3D_TILE_DISTRIBUTION_TASKS
get_missing_target_fill_value = _malibu3d_cfg.get_missing_target_fill_value
get_semantic_config = _malibu3d_cfg.get_semantic_config
get_multilabel_classification_config = _malibu3d_cfg.get_multilabel_classification_config

SEMANTIC_TASKS = tuple(MALIBU3D_SEMANTIC_TARGET_KEYS)
NATHAB_AXIS_TASKS = tuple(MALIBU3D_TILE_DISTRIBUTION_TASKS.keys())
...
def _load_label_remap_module():
    module_name = "malibu3d_label_remap_count_script"
    path = os.path.join(
        REPO_ROOT,
        "pointcept",
        "datasets",
...
def _default_storage_definition_name(task: str, get_default_definition_name) -> str:
    ...
    *training-target* definition (get_default_definition_name), which happen to coincide
    ...
    required. See also get_missing_target_fill_value's docstring in malibu3d_config_utils.py
...
def build_task_remap_state(
    ...
    """Mirror Malibu3DLabelRemap: storage on-disk defs -> target training defs."""
...
def build_nathab_axis_remap_state(
    ...
    state: Dict[str, TaskRemapState] = {}
    for axis, target_name in MALIBU3D_TILE_DISTRIBUTION_TASKS.items():
...
def load_hardcoded_excluded_tiles() -> set[tuple[str, str]]:
    """Malibu3DDataset.get_hardcoded_excluded_tiles() (CORRUPTED_TILES + missing coord CSV)."""
    excluded = set()
    details_csv = os.path.join(REPO_ROOT, "data", "malibu3d_plus", "missing_coord_tiles.details.csv")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not apply Malibu3DDataset hardcoded/missing-coord exclusions.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "(default: use ignore fill like Malibu3DDataset).",
    )
    parser.add_argument(
        "--label_definitions",
        default="",
        help="Comma-separated task=definition for training label defs "
        "(e.g. natural_habitat=by_moisture_v3). Defaults match malibu3d_label_remap.",
    )
    parser.add_argument(
        "--storage_definitions",
        default="",
        ...
        "Defaults match malibu3d_label_remap defaults.",
    )
    ...
    parser.add_argument(
        "--output_dir",
        default="stats/malibu3d/label_distribution",
