...
# DEBUG smoke: Sonata linear probe on Flair3D+ segment (2× A100, ~20′).
# Usage:
#   sbatch scripts/sonata/sbatch_lin_probe_debug.sh <weight.pth> [exp_name]
# Or with env vars (LinProbeSbatchHook / watcher):
#   WEIGHT=... EXP_NAME=... PRETRAIN_JOB_DIR=... PRETRAIN_EPOCH=... PRETRAIN_ITERS=... \
#     sbatch scripts/sonata/sbatch_lin_probe_debug.sh
#
...
#SBATCH --job-name=sonata_lin_dbg

WEIGHT="${WEIGHT:-${1:?WEIGHT path required (arg1 or WEIGHT=)}}"
EXP_NAME="${EXP_NAME:-${2:-sonata_lin_probe_debug}}"
...

{
    ...
    echo "Config: flair3d_default/segment/sonata-v1m2-flair3d-lin-debug"
...
sh scripts/train.sh \
  -g 2 \
  -d flair3d_default \
  -c segment/sonata-v1m2-flair3d-lin-debug \
  -n "$EXP_NAME" \
  -w "$WEIGHT" || TRAIN_RC=$?
