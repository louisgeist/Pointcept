...
# Sonata-v1m2 pretrain on Flair3D+ from a chosen config (default: 1 node × 8 A100).
# Same as sbatch_pretrain.sh but with a selectable config, for A/B'ing config
# variants (e.g. configs/experiment/w107/1/sonata_vram/*) from scratch.
#
# Usage: sbatch scripts/sonata/sbatch_pretrain_variant.sh <exp_name> <config_rel_no_ext>
# Example:
#   sbatch scripts/sonata/sbatch_pretrain_variant.sh \
#     sonata_vram_bs1 experiment/w107/1/sonata_vram/bs1
#
...
#SBATCH --job-name=sonata_pretrain_variant

EXP_NAME="${1:?usage: sbatch sbatch_pretrain_variant.sh <exp_name> <config_rel_no_ext>}"
CONFIG_REL="${2:?usage: sbatch sbatch_pretrain_variant.sh <exp_name> <config_rel_no_ext>}"
