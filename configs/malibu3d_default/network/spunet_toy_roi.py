"""
SpUNet ROI overfit for network pixel segmentation on local machine (D067).

Same recipe as ``spunet_toy_long.py``, but overfits on the full
D067-2021_AF-S1-22 ROI (100 tiles, all locally mirrored on local machine) instead of
...
Example::
    ...
    python tools/train.py --config-file configs/malibu3d_default/network/spunet_toy_roi.py \\
      --num-gpus 1
"""
...
csv_manifest = "data/malibu3d_plus/raw/scene_split_manifest_D067-2021_AF-S1-22.csv"
...
network_apls_eval = dict(
    network_graphs_root="data/malibu3d_build/data/network_graphs",      split="train",
    threshold=0.2,
