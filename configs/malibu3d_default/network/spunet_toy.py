"""
SpUNet toy overfit for network pixel segmentation on local machine (D067).

One unique train tile (``max_sample=1``), repeated via ``loop`` so the DataLoader
can form full batches. No crop (whole tile every step, batch_size=1) and light
augs so the run can overfit quickly and prove the pipeline (Mix3D off,
point-wise network labels, dense test save).

Example::

    export PYTHONPATH="$PWD"
    python tools/train.py --config-file configs/malibu3d_default/network/spunet_toy.py \\
      --num-gpus 1
"""
...
wandb_run_name = (
    f"SpUNet network overfit D067 | lr={lr}, mix_prob={mix_prob}"
)
wandb_project = "malibu3d_network_overfit"
...
train_multitask_keys, val_multitask_keys, multitask_index_valid_keys = (
    init_multitask_collect_keys(
        target_keys, collect_prefix_keys=MALIBU3D_COLLECT_PREFIX_GRID
    )
)

del MALIBU3D_COLLECT_PREFIX_GRID, init_multitask_collect_keys
