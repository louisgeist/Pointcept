"""
Toy SpUNet config for debugging nathab axis-distribution training on local machine (D067 subset).
...
wandb_project = "malibu3d_nathab_distribution"
...
task_target_keys = tuple(MALIBU3D_TILE_DISTRIBUTION_TASKS.keys())
main_task = "nathab_habitat_type"

# `natural_habitat` is loaded raw (loader-only, not itself a supervised task) so
# Malibu3DLabelRemap's fan-out below has a source field to read from; it is
...
train_multitask_keys, val_multitask_keys, multitask_index_valid_keys = (
    init_multitask_collect_keys(
        task_target_keys, collect_prefix_keys=MALIBU3D_COLLECT_PREFIX_GRID
    )
)
