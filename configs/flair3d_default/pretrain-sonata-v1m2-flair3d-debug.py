"""
DEBUG smoke: Sonata-v1m2 pretrain on Flair3D+ (2× A100, few iters).

Same model/data as pretrain-sonata-v1m2-flair3d.py, but short schedule +
max_sample. LinProbeSbatchHook stays enabled and submits
scripts/sonata/sbatch_lin_probe_debug.sh once (at epoch_2).
"""
...
wandb_run_name = (
    f"DEBUG | Sonata-v1m2 pretrain Flair3D+ | bs={batch_size} | grid={grid_size} | "
    f"iters={total_iters}"
)
...
hooks = [
    ...
    # After CheckpointSaver; submits one debug lin-probe at epoch_2.
    dict(
        type="LinProbeSbatchHook",
        enable=True,
        save_freq=2,
        sbatch_script="scripts/sonata/sbatch_lin_probe_debug.sh",
