"""
Sonata-v1m2 grid-search linear probe on Flair3D+ segment (v20) — Jean Zay experiment w108/4/gridprobe (1k).

Explicit nested-loop cartesian (no cartesian_probes helper): loss × lr × wd ×
input_norm (unitsphere), all AdamW + OneCycleLR. Shared frozen PT-v3m2
forward once per batch — see pointcept/models/grid_probe.py and
GridProbeTrainer.

Intended checkpoint (pretrain job 862680, epoch 76):
...
Grid (72 probes):
  3 losses (ce_lovasz, ce, focal_g2)
  × 3 LRs (1e-3, 2e-3 default, 5e-3)
  × 2 weight decays (0, 2e-2)
...
wandb_run_name = f"Sonata-v1m2 grid-probe Flair3D+ segment | iters={total_iters} | ep76"
...
data = dict(
    ...
    val=dict(
        ...
        # Wider than periodic lin-probe (20): stabler ranking across probes.
        max_sample=100,
        target_keys=["segment"],
        primary_target_key="segment",
        transform=[
