...
GradNormLite pooled nathab) except target_keys swaps "forest" for "forest_2d",
and the train/val/test pipelines gain ExtractAbsXY + NetworkRasterToPointLabels
(target_key="forest_2d"), mirroring how w107/4/18h/multi-litept-v1m0-flair3d_3
wires the "network" pixel-semantic task.

Tasks: segment (v20) + forest_2d + elevation + 4 nathab tile_distribution axes
...
grid_size = 0.1
point_max = 102400
# Mix3D OK: NetworkRasterToPointLabels makes forest_2d point-wise before collate.
mix_prob = 0.8
...
# Wandb parameters
wandb_run_name = (
    f"Flair3D+ LitePT-S multi + forest_2d + nathab_distribution "
    f"{grp_exp}.{num_exp} stride={stride} batch_size={batch_size} lr={lr} "
    f"grad_norm_lite=pooled_nathab elevation=meters"
)
...
# scaling each independently (see module docstring). segment/forest_2d/elevation
...
# beta=1.0 in meters ≡ former ELEVATION_SMOOTH_L1_BETA=1e-2 in ×0.01 space (~1 m).
...
# tile_distribution: WeightedKLDivLoss on pooled softmax; forest_2d: pixel_semantic).
model = dict(
