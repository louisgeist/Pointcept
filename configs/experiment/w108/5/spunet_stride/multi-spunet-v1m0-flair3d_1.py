...
_1: same as flair3d_default/multi-spunet-v1m0-flair3d.py with total_iters=50_000
and pooling stride=3 on every existing downsample (4 stages, not more/fewer).
...
# Pooling stride applied to each of the 4 existing SparseConv downsamples.
# Does not change the number of pooling layers (default architecture is 4).
stride = 3
...
# Wandb parameters
wandb_run_name = (
    f"SpUNet multi {grp_exp}.{num_exp} iters={total_iters} stride={stride}"
)
