...
learned_masked_feat = False
wandb_run_name = f"KPConvX {grp_exp}.{num_exp}) base - input=RGB+XYZ+strength | no RandomRotate(x,y) | feat_masking + LearnMaskedValue={learned_masked_feat}"
...
model = dict(
    ...
    backbone=dict(
        type="kpconvx_base",
        input_channels=7, # RGB + Coords + strength
...
data = dict(
    ...
    test=dict(
        ...
        test_cfg=dict(
            ...
            aug_transform=[[dict(type="RandomRotateTargetAngle", angle=[0], axis="z", center=[0, 0, 0], p=1)]], # no TTA
        )
    ),
)
