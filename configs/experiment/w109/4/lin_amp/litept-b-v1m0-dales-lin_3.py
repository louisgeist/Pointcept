...
AMP variant of litept-b-v1m0-dales-lin_1.py: that config OOM'd on job
1200400 (fp32, 1404ch hypercolumn feat shared across 24 probes, 16 of which
have dropout>0 — each such probe needs its own fp32 copy of the shared
[P, 1404] backbone feat for its Dropout+Linear backward, and GridProbeTrainer
keeps every active probe's forward+loss graph alive until one shared
backward() over the summed loss, see GridProbeTrainer docstring in
pointcept/engines/train.py). Only change here is `enable_amp = True`, halving
those per-probe activation copies (fp16 vs fp32) — testing whether that's
enough headroom on an 80GB GPU without touching the probe grid/point_max.
...
# (1 x 4 x 2 x 3 x 1 = 24 probes). Same grid as sibling ptv3-v1m0-dales-lin_1.py.
# -----------------------------------------------------------------------------
_losses = {
    "ce_lovasz": [
...
wandb_run_name = (
    ...
    f"AMP, bn_eval_mode=False, {len(probes)} probes, epoch={epoch}"
)
