...
finest-stage-dropped ablation of litept-b-v1m0-dales-lin_1.py (same frozen
checkpoint, job 873542, Flair3D+ multitask supervised pretrain).

Motivation: the finest decoder stage (dec_channels[0]=72ch, the layer
immediately adjacent to the original supervised seg_head) may sit at a very
different activation scale than the rest of the 1404ch hypercolumn and/or
carry pretrain-task-specific bias, hurting a plain (feat_norm=none) linear
probe. `drop_leading_channels=72` (new GridProbeSegmentorV2 knob, see
pointcept/models/grid_probe.py) drops exactly that stage from every probe's
input — concat order is always finest-stage-first (see _forward_backbone
docstring) — leaving 1332ch (108+216+432+576). Backbone is unchanged
(dec_traceable=True, still computes all stages); only the probe input is
sliced.

Grid (21 probes): ce_lovasz x lr{2e-3,5e-3,1e-2,2e-2,5e-2} x wd{0,5e-3} x
dropout=0 x input_norm=None x feat_norm=none x optimizer=AdamW x
warmup{5%,10%} (OneCycleLR pct_start), plus one extra single probe at
feat_norm=batchnorm / lr=2e-2 / wd=5e-3 / warmup=5%. epoch=200 / eval_epoch=10,
num_worker=16 (H100). Same lr/wd/warmup grid as the other grid_20h configs.
AMP enabled (fp16). Verify VRAM with scripts/sonata/diagnose_grid_probe_vram.py
before submitting, don't assume it fits.
...
# Decoder levels (72, 108, 216, 432) + raw encoder bottleneck (576). Backbone
# still computes/returns the full 1404ch hypercolumn; drop_leading_channels
# below strips dec_channels[0] (72ch, concatenated first) before the probes.
dec_channels = (72, 108, 216, 432)
bottleneck_channels = 576
backbone_out_channels = sum(dec_channels) + bottleneck_channels  # 1404
drop_leading_channels = dec_channels[0]  # 72 -> probes see 1332ch

# -----------------------------------------------------------------------------
# Grid-search probes — ce_lovasz x lr x wd x dropout x input_norm x feat_norm x
# optimizer x warmup (1 x 5 x 2 x 1 x 1 x 1 x 1 x 2 = 20 probes).
# -----------------------------------------------------------------------------
_losses = {
    "ce_lovasz": [
...
# Extra single probe: feat_norm=batchnorm (unswept elsewhere in this grid) at
# lr=2e-2 / wd=5e-3 / warmup=5% — ablates whether normalizing the 1332ch
# hypercolumn (dec_channels[0] already dropped) before the linear probe
# corrects any remaining cross-stage scale heterogeneity.
probes["ce_lovasz_lr2e-2_wd5e-3_do0_none_fnbn_adamw_w5"] = dict(
...
wandb_run_name = (
    f"LitePT-B GridProbe DALES {grp_exp}.{num_exp}) decoder hypercolumn drop dec[0] "
    f"({backbone_out_channels - drop_leading_channels}ch of {backbone_out_channels}) H100, "
    f"lower lr sweep, {len(probes)} probes, epoch={epoch}"
)
