...
configs/dales/litept-b-v1m0-dales-lin-grid-enc.py). `enc_mode=True` drops the
...
stages (54+108+216+432+576) — same mechanism as this backbone's Sonata
counterpart's 1232ch feat.

H100 rerun of w109/4/grid_19h/litept-b-v1m0-dales-lin_2.py: lower/finer lr
range, epoch=200 (up from 100), num_worker=16 (H100). Drops that config's
one-off feat_norm=layernorm smoke probe (orthogonal to this sweep).
...
# lr=2e-2 / wd=5e-3 / warmup=5% — ablates whether normalizing the 1386ch
# hypercolumn before the linear probe corrects the cross-stage scale
# heterogeneity between the finest encoder stage and the bottleneck.
