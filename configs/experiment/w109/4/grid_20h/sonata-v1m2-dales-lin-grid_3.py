"""
Sonata-v1m2 grid-search linear probing on DALES — H100 rerun of
configs/dales/sonata-v1m2-dales-lin-grid.py with the lower/finer lr range
shared with w109/4/grid_20h/litept-b-v1m0-dales-lin_{1,2}.py.
...
dropout=0 x input_norm=None x warmup{5%,10%} (OneCycleLR pct_start), plus one
extra single probe at feat_norm=batchnorm / lr=2e-2 / wd=5e-3 / warmup=5%
(feat_norm=none everywhere else in the grid).
epoch=200 (up from 50), eval_epoch=10, num_worker=16 (H100). skip_test=True
(val-only winner selection).
"""
...
# -----------------------------------------------------------------------------
# Grid-search probes — ce_lovasz x lr x wd x dropout x input_norm x warmup
# (1 x 5 x 2 x 1 x 1 x 2 = 20 probes).
# -----------------------------------------------------------------------------
_losses = {
    "ce_lovasz": [
...
# lr=2e-2 / wd=5e-3 / warmup=5% — ablates whether normalizing the 1232ch
# hypercolumn before the linear probe corrects the cross-stage scale
# heterogeneity discussed for the LitePT-B decoder-hypercolumn configs.
probes["ce_lovasz_lr2e-2_wd5e-3_do0_none_bn_w5"] = dict(
...
del _losses, _lrs, _wds, _dropouts, _norms, _warmups, _loss_name, _criteria, _lr_name, _lr
del _wd_name, _wd, _do_name, _dropout, _norm_name, _input_norm, _wu_name, _pct_start, _name

wandb_run_name = (
    f"Sonata GridProbe DALES {grp_exp}.{num_exp}) epoch_120, enc multiscale 1232ch H100, "
