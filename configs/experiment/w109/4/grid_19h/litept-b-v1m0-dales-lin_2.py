...
Sibling of litept-b-v1m0-dales-lin_1.py (decoder hypercolumn): same wider
sweep (feat_norm, lr, optimizer, epoch=100), same probe grid, only the
tapped feature differs. Point: with both the feature-source ablation
(enc vs dec) and the wider optimization sweep in place at once, isolate
whether closing the Sonata GridProbe DALES head-start gap is mostly about
which features get probed, or about how the probe itself is trained.
...
AMP enabled (fp16). VRAM: 72 probes is a 3x jump over the reference dales
grid configs (16-24 probes) even under AMP; verify with
scripts/sonata/diagnose_grid_probe_vram.py before submitting, don't assume
it fits.
...
wandb_run_name = (
    f"LitePT-B GridProbe DALES {grp_exp}.{num_exp}) enc multiscale {backbone_out_channels}ch, "
