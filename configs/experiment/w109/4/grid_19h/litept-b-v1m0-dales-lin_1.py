...
(same frozen checkpoint as configs/dales/litept-b-v1m0-dales-lin-grid.py, job
873542, Flair3D+ multitask supervised pretrain). `traceable`-free: LitePT-v1
here uses `dec_traceable=True` → 1404ch concat of decoder stages + bottleneck.

Wider sweep than the reference dales grid config, probing three follow-ups
from the GridProbe DALES head-start discussion (Sonata >> PTv3/LitePT at
epoch 1): per-channel feature normalization (`feat_norm`, batchnorm/layernorm
— untested so far, distinct from the already-ruled-out `input_norm`
unit-sphere rescale, which does not correct for per-channel scale
heterogeneity across concatenated stages), a lower/finer lr range, and an
SGD-vs-AdamW optimizer axis (SGD momentum=0.9, same OneCycleLR schedule).
`epoch=100` (up from 50) since the decoder-hypercolumn probes were still
climbing at the last eval tick of the 50-epoch runs.

Grid (72 probes): ce_lovasz x lr{1e-2,2e-2,5e-2} x wd{5e-3,5e-2} x
dropout{0,0.3} x input_norm=None x feat_norm{none,batchnorm,layernorm} x
optimizer{AdamW,SGD}. epoch=100 / eval_epoch=10.
AMP enabled (fp16). VRAM: sibling reference configs OOM'd in fp32 at 24
probes on this backbone's 1404ch hypercolumn (see GridProbeTrainer docstring
in pointcept/engines/train.py) — 72 probes is a 3x jump even under AMP;
verify with scripts/sonata/diagnose_grid_probe_vram.py before submitting,
don't assume it fits.
...
# optimizer (1 x 3 x 2 x 2 x 1 x 3 x 2 = 72 probes).
# -----------------------------------------------------------------------------
_losses = {
    "ce_lovasz": [
...
_lrs = {"1e-2": 1e-2, "2e-2": 2e-2, "5e-2": 5e-2}
_wds = {"5e-3": 0.005}
_dropouts = {"0": 0.0}
_norms = {"none": None}
...
# Extra smoke probe: ce_lovasz x lr=2e-2 x wd=5e-3 x do=0 x input_norm=none x
# feat_norm=layernorm x AdamW — not covered by the grid above, which only
# sweeps feat_norm=none.
...
wandb_run_name = (
    f"LitePT-B GridProbe DALES {grp_exp}.{num_exp}) decoder hypercolumn {backbone_out_channels}ch, "
    f"lr/feat_norm/optimizer sweep, {len(probes)} probes, epoch={epoch}"
)
