...
litept-b-v1m0-dales-lin_1/_2.py in this directory (same DINOv2 linear-probing
protocol replication: SGD momentum=0.9, no weight decay, no warmup,
CosineAnnealingLR, plus the none/l2/linf whole-vector input_norm axis).
...
same checkpoint as w109/4/grid_20h/sonata-v1m2-dales-lin-grid_3.py. DALES has
no RGB — Sonata was pretrained with scene-level RandomDropColor/
RandomDropStrength (drop_value=0.0) so `FillMissingFeat` synthesizes a zero
"color" channel (in_channels=7). No learned masked-feat at pretrain time, so
...
over the full run. epoch=200 / eval_epoch=10, num_worker=24 (H100) — same as
the LitePT-B configs in this directory.
...
feat_norm=none x optimizer=SGD (no warmup axis). AMP enabled (fp16). Verify
VRAM with scripts/sonata/diagnose_grid_probe_vram.py before submitting, don't
assume it fits.
"""
