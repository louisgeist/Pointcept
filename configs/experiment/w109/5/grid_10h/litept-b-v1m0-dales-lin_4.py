...
(same frozen checkpoint as litept-b-v1m0-dales-lin_1.py / job 873542, and as
w109/4/grid_20h/litept-b-v1m0-dales-lin_2.py). `enc_mode=True` drops the
decoder; GridProbeSegmentorV2's encoder-side multiscale concat (walking
`pooling_parent` links) hands the probes a 1386ch concat of the 5 raw encoder
stages (54+108+216+432+576).

Per-block feature normalization: instead of unit-sphere-rescaling the whole
1386ch multiscale concat as one vector (the l2/linf input_norm swept in
litept-b-v1m0-dales-lin_2.py), `channel_blocks` (new
GridProbeSegmentorV2/ProbeHead knob, see pointcept/models/grid_probe.py)
tells the model where each encoder level's boundaries are — here
enc_channels = (54, 108, 216, 432, 576), in the same finest-stage-first
concat order as _forward_backbone — so input_norm kinds ending in "_block"
(l2_block/linf_block) normalize each level's own sub-vector independently
before re-concatenating, instead of the pooled multiscale feat as a whole.

AdamW / wd=0 / OneCycleLR (pct_start=5%) — same optimizer/scheduler family as
the grid_19h/grid_20h AdamW probes, just re-run here to compare against the
new per-block input_norm. A handful of lr test points rather than the full
grid_20h lr sweep, since this is primarily about validating the block-norm
axis, not re-sweeping lr. epoch=200 / eval_epoch=10, num_worker=24 (H100) —
same as litept-b-v1m0-dales-lin_1/_2.py in this directory.

Grid (15 probes): ce_lovasz x lr{2e-3,5e-3,1e-2,2e-2,5e-2} x wd=0 x
dropout=0 x input_norm{none,l2_block,linf_block} x feat_norm=none x
optimizer=AdamW, warmup=5% (OneCycleLR pct_start). AMP enabled (fp16).
Verify VRAM with scripts/sonata/diagnose_grid_probe_vram.py before
submitting, don't assume it fits.
...
# Encoder levels (enc_mode), finest-first concat order — also the block
# boundaries for the "*_block" input_norm kinds (see channel_blocks below).
enc_channels = (54, 108, 216, 432, 576)
backbone_out_channels = sum(enc_channels)
channel_blocks = enc_channels

# -----------------------------------------------------------------------------
# Grid-search probes — ce_lovasz x lr x wd=0 x dropout=0 x
# input_norm{none,l2_block,linf_block} x feat_norm=none x optimizer=AdamW,
# warmup=5% (1 x 5 x 1 x 1 x 3 x 1 x 1 = 15 probes).
# -----------------------------------------------------------------------------
_losses = {
    "ce_lovasz": [
...
_lrs = {"2e-3": 2e-3, "5e-3": 5e-3, "1e-2": 1e-2, "2e-2": 2e-2, "5e-2": 5e-2}
_wds = {"0": 0.0}
_dropouts = {"0": 0.0}
_norms = {"none": None, "l2_block": "l2_block", "linf_block": "linf_block"}
...
wandb_run_name = (
    f"LitePT-B GridProbe DALES {grp_exp}.{num_exp}) enc multiscale {backbone_out_channels}ch H100, "
    f"per-block input_norm (AdamW/wd0/warmup5%), {len(probes)} probes, epoch={epoch}"
)
