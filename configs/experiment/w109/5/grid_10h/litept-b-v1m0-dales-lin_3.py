...
(same frozen checkpoint as w109/4/grid_20h/litept-b-v1m0-dales-lin_1.py, job
873542, Flair3D+ multitask supervised pretrain). `dec_traceable=True` → 1404ch
concat of decoder stages + bottleneck.

Per-block feature normalization: instead of unit-sphere-rescaling the whole
1404ch hypercolumn as one vector (the l2/linf input_norm swept in
litept-b-v1m0-dales-lin_1.py), `channel_blocks` (new
GridProbeSegmentorV2/ProbeHead knob, see pointcept/models/grid_probe.py)
tells the model where each concatenated stage's boundaries are — here
dec_channels + (bottleneck_channels,) = (72, 108, 216, 432, 576), in the same
finest-stage-first concat order as _forward_backbone — so input_norm kinds
ending in "_block" (l2_block/linf_block) normalize each stage's own
sub-vector independently before re-concatenating, instead of the pooled
hypercolumn as a whole.
...
# Decoder levels (72, 108, 216, 432) + raw encoder bottleneck (576), in
# finest-stage-first concat order — also the block boundaries for the
# "*_block" input_norm kinds (see channel_blocks on the model below).
dec_channels = (72, 108, 216, 432)
bottleneck_channels = 576
backbone_out_channels = sum(dec_channels) + bottleneck_channels  # 1404
channel_blocks = dec_channels + (bottleneck_channels,)  # (72, 108, 216, 432, 576)
