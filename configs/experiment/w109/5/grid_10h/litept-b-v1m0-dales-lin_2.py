...
DINOv2 linear-probing protocol replication (see DINOv2 paper's linear-eval
recipe): SGD (momentum=0.9), no weight decay, no warmup, cosine-annealed LR
swept over {1e-4 .. 5e-1} log-ish spaced. CosineAnnealingLR (eta_min=0) is
built per eval_epoch window like every other scheduler in this GridProbe
family (GridProbeTrainer.build_scheduler injects total_steps per window, see
pointcept/engines/train.py) — not a single decay over the full run. Same
epoch=200 / eval_epoch=10 as grid_20h/litept-b-v1m0-dales-lin_2.py (only the
optimizer/lr/scheduler/input_norm axes change here); loss/dropout/feat_norm
stay fixed at the grid_20h defaults (ce_lovasz / 0 / none) and the one-off
feat_norm=batchnorm smoke probe from grid_20h is dropped — orthogonal to this
sweep. input_norm now sweeps the row-wise unit-sphere rescale (None/l2/linf,
see `_unitsphere` in pointcept/models/grid_probe.py) crossed with the lr
sweep — grid_19h had already ruled this out under the AdamW/OneCycleLR
protocol, re-checking here under SGD/no-warmup/cosine.
...
feat_norm=none x optimizer=SGD (no warmup axis). epoch=200 / eval_epoch=10,
num_worker=24 (H100). AMP enabled (fp16). Verify VRAM with
scripts/sonata/diagnose_grid_probe_vram.py before submitting, don't assume it
fits (39 SGD probes vs grid_20h's 21 AdamW probes — SGD-momentum keeps one
optimizer state buffer per param vs AdamW's two, but the probe count is up
~1.9x).
