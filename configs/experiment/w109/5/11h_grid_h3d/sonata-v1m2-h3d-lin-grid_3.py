"""
Sonata-v1m2 grid-search linear probing on H3D — Sonata counterpart of
litept-b-v1m0-h3d-lin_1/_2.py in this directory (same DINOv2 linear-probing
protocol replication: SGD momentum=0.9, no weight decay, no warmup,
CosineAnnealingLR, plus the none/l2/linf whole-vector input_norm axis) and of
the DALES sonata sweep, w109/5/grid_10h/sonata-v1m2-dales-lin-grid_5.py.

Frozen PT-v3m2 encoder (enc_mode=True -> multi-scale concat 1232ch =
48+96+192+384+512) from the Flair3D+ Sonata pretrain job 862680, epoch_120 —
same checkpoint as w109/4/h3d_lin/sonata-v1m2-h3d-lin_1.py. H3D has no
strength (LAS `intensity` is all-zero for this release; `Reflectance` is on a
different scale and not extracted by preprocess_h3d.py). Sonata was
pretrained with scene-level RandomDropColor/RandomDropStrength
(drop_value=0.0), so `FillMissingFeat` synthesizing a zero "strength" channel
is in-distribution (mirrors DALES's zero "color" channel — same mechanism,
opposite missing modality). No learned masked-feat at pretrain time, so
literal zero fill is faithful (no feature_mask_values needed, unlike the
LitePT-B configs). No coord_feat_scale — Sonata pretrain doesn't use it (see
CLAUDE.md GridProbe notes).

CosineAnnealingLR (eta_min=0) is built per eval_epoch window like every other
scheduler in this GridProbe family (GridProbeTrainer.build_scheduler injects
total_steps per window, see pointcept/engines/train.py) — not a single decay
over the full run.

Dataset-driven axes (epoch/num_worker/AMP) come from H3D's own established
GridProbe config (sonata-v1m2-h3d-lin_1.py) rather than the DALES grid_10h
sibling: epoch=800 (vs DALES's 200) to reach a comparable step budget on
H3D's much smaller split (15 train / 4 val / 11 test tiles), num_worker=
8*num_gpu (not H100). AMP is enabled here though (the H3D reference used
enable_amp=False) because this sweep goes from that reference's 24 probes to
39 (same ~1.6x jump that forced grid_10h to switch DALES to AMP at the same
probe count) on the same 1232ch encoder width — verify VRAM with
scripts/sonata/diagnose_grid_probe_vram.py before submitting.
