...
(same frozen checkpoint as litept-b-v1m0-h3d-lin_1.py / job 873542, and as
the DALES counterpart w109/5/grid_10h/litept-b-v1m0-dales-lin_2.py).
`enc_mode=True` drops the decoder; GridProbeSegmentorV2's encoder-side
multiscale concat (walking `pooling_parent` links) hands the probes a 1386ch
concat of the 5 raw encoder stages (54+108+216+432+576).
...
DALES grid_10h siblings — see litept-b-v1m0-h3d-lin_1.py in this directory
for the full rationale (epoch=800, num_worker=8*num_gpu, AMP enabled to
absorb the 24->39 probe jump on the same 1386ch multiscale width). Verify
VRAM with scripts/sonata/diagnose_grid_probe_vram.py before submitting.

H3D has real RGB but no strength (opposite of DALES) — see
litept-b-v1m0-h3d-lin_1.py for the FillMissingFeat / Chromatic augmentation /
feature_mask_values rationale, unchanged here except for the encoder-side
backbone.
...
wandb_run_name = (
    f"LitePT-B GridProbe H3D {grp_exp}.{num_exp}) enc multiscale {backbone_out_channels}ch, "
