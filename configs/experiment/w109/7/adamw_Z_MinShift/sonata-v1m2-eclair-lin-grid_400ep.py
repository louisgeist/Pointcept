"""
Sonata-v1m2 grid-search linear probing on ECLAIR — Z_MinShift variant of
w109/6/3_grid_eclair/sonata-v1m2-eclair-lin-grid_400ep.py (same frozen
checkpoint, job 862680, epoch_120). Frozen PT-v3m2 encoder (enc_mode=True →
multi-scale concat 1232ch). ECLAIR provides real RGB (normalized via
NormalizeColor); strength uses the same 1/60000 scale as DALES. No
coord_feat_scale (Sonata pretrain does not use it). Jean-Zay H100
(num_worker=24).

Z_MinShift/Z_RandomOffset added to match the Flair3D+ pretrain pipeline: train
uses both; val/test use Z_MinShift only (no Z_RandomOffset).
...
wandb_run_name = (
    ...
    f"AdamW/wd0/OneCycleLR warmup5%, Z_MinShift, {len(probes)} probes, epoch={epoch}"
)
