"""
Sonata-v1m2 grid-search linear probing on H3D — wd=0.02 variant of
w109/7/grid_sonata_shorter/sonata-v1m2-h3d-lin-grid_1.py (same frozen
...
train->val/test gap seen on H3D relative to LitePT-B at comparable train
mIoU.
...
AdamW / wd=0.02 / OneCycleLR with warmup fixed at pct_start=0%, lr swept over
{1e-4 .. 5e-1} (12 values), input_norm=none. epoch=1000 / eval_epoch=10
(half the 2000ep grid_h3d schedule, unchanged from the shorter variant).
...
wandb_run_name = (
    ...
    f"AdamW/wd0.02/OneCycleLR warmup0%, {len(probes)} probes, epoch={epoch}"
)
