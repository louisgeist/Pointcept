"""
Sonata-v1m2 grid-search linear probing on ECLAIR — wd=0.02 variant of
w109/6/3_grid_eclair/sonata-v1m2-eclair-lin-grid_400ep.py (same frozen
checkpoint, job 862680, epoch_120). All 12 sweep configs in this repo
(H3D/DALES/ECLAIR) fixed wd=0 to isolate the LR axis, but every canonical
Sonata lin-probe config (configs/sonata/semseg-sonata-v1m1-*-lin.py:
ScanNet/ScanNet200/ScanNet++/S3DIS/AEO) and the in-domain Flair3D reference
(configs/flair3d_default/probe/sonata-v1m2-flair3d-lin.py) use wd=0.02 —
this reverts to that canonical value to check whether it closes the large
train->val/test gap seen on H3D (and by extension ECLAIR) relative to
LitePT-B at comparable train mIoU.
...
No coord_feat_scale (Sonata pretrain does not use it). Jean-Zay H100
(num_worker=24).

Grid (12 probes): ce_lovasz x 12 LRs x wd=0.02 x dropout=0 x
input_norm=None x AdamW x warmup=5%. epoch=400 / eval_epoch=10.
...
# -----------------------------------------------------------------------------
# Grid-search probes — AdamW / OneCycleLR: ce_lovasz x lr x wd=0.02 x dropout=0 x
# input_norm=none x feat_norm=none x optimizer=AdamW, warmup=5%
# (1 x 12 x 1 x 1 x 1 x 1 x 1 = 12 probes). Same grid as LitePT sibling.
# -----------------------------------------------------------------------------
_losses = {
    "ce_lovasz": [
...
wandb_run_name = (
    f"Sonata GridProbe ECLAIR {grp_exp}.{num_exp}) epoch_120, enc multiscale 1232ch H100 ep400, "
    f"AdamW/wd0.02/OneCycleLR warmup5%, {len(probes)} probes, epoch={epoch}"
)
