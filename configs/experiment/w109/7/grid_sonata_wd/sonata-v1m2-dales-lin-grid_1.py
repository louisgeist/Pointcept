"""
Sonata-v1m2 grid-search linear probing on DALES — wd=0.02 variant of
w109/5/grid_dales_19h/sonata-v1m2-dales-lin-grid_3.py (same frozen
...
train->val/test gap seen on H3D (and by extension DALES) relative to
...
AdamW / wd=0.02 / OneCycleLR with warmup fixed at pct_start=5%, lr swept over
{1e-4 .. 5e-1} (12 values), input_norm=none. epoch=400 / eval_epoch=10,
num_worker=24 (H100) — unchanged from the wd=0 source.

Grid (12 probes): ce_lovasz x lr{1e-4,2e-4,5e-4,1e-3,2e-3,5e-3,1e-2,2e-2,5e-2,
1e-1,2e-1,5e-1} x wd=0.02 x dropout=0 x input_norm=none x feat_norm=none x
optimizer=AdamW x warmup=5%.
"""
