"""
Sonata-v1m2 linear probing on Flair3D+ segment (v20) — Jean Zay experiment
w109/1/sonata_lin (_1).

Single-head recipe copied from the grid-probe winner on pretrain epochs 9 and
76: CE only, lr=2e-3, weight_decay=0, input_norm=None (DefaultSegmentorV2 has
no unitsphere norm). Frozen PT-v3m2 encoder (enc_mode); CheckpointLoader
remaps student.backbone → backbone.

Checkpoint (pretrain job 862680, epoch 143):
  /lustre/fsn1/projects/rech/unv/usi32yh/logs/pointcept_logs/slurm/862680/model/epoch_143.pth
...
# Sonata pretrain ckpt (job 862680, epoch 143) — remap via CheckpointLoader
weight = "/lustre/fsn1/projects/rech/unv/usi32yh/logs/pointcept_logs/slurm/862680/model/epoch_143.pth"
...
wandb_run_name = (
    f"Sonata-v1m2 lin probe FT {grp_exp}.{num_exp} | ce_lr2e-3_wd0_none | "
    f"ep143 | bs={batch_size} | iters={total_iters}"
)
...
# -----------------------------------------------------------------------------
# Optimizer / scheduler — grid-probe winner ce_lr2e-3_wd0_none
# -----------------------------------------------------------------------------
lr = 2e-3
...
# Hooks
# -----------------------------------------------------------------------------
# No PreciseEvaluator / test split: probes only need best val mIoU
# (written by MetricsJsonWriter → metrics.json).
hooks = [
    dict(
