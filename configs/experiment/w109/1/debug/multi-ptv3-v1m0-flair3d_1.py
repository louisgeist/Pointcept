"""
PT-v3-malibu on Flair3D+ multitask — short H100 debug smoke (w109/1/debug).

Same recipe as w109/1/ptv3_lr_base_200k/multi-ptv3-v1m0-flair3d_2.py
(segment v20 + forest_2d + elevation + 4 nathab axes + network roads-only,
lr=2e-3, H100). Debug-speed overrides only: few train iters, a handful of
val/test scenes, eval_every=1, no val_stratified_subset_manifest (rely on
max_sample).
...
total_iters = 6
iter_per_epoch = 2
eval_every = 1  # validate after each trainer epoch (3 epochs = 6 / 2)
...
# Wandb parameters
wandb_run_name = (
    f"DEBUG H100 ptv3 multi {grp_exp}.{num_exp} iters={total_iters} lr={lr}"
)
