...
_4: ablation vs the Flair3D multi default (kp_radius=kp_sigma=3.2,
radius_scaling=3.0): keep the ScanNet / KPConvX-paper recipe
(kp_radius=kp_sigma=2.3, radius_scaling=2.2). 8x A100, 3 samples/GPU
(OOM at 4), global batch_size=24. Val/test voxel budget 1M, cap 8 scenes/GPU. 200k iterations.
...
# Data parameters
# Train batch_size is the global effective batch (3 per GPU when num_gpu=8).
...
batch_size = 24  # total across GPUs (3 per GPU when num_gpu=8)
...
lr = 5e-3 #+wd : taken from scannet config
total_iters = 200_000
...
# Wandb parameters
wandb_run_name = (
    f"8xA100 KPConvX multi {grp_exp}.{num_exp} iters={total_iters} bs={batch_size} rs={radius_scaling} lr={lr}"
)
