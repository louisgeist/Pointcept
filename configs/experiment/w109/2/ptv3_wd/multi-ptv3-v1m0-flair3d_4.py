...
_4: same as w109/1/ptv3_lr_base_200k/multi-ptv3-v1m0-flair3d_2.py with
total_iters=200_000, lr=1e-3, outdoor-style weight_decay=5e-3, 4x H100
(effective train batch stays 12 total / 3 per GPU; val/test caps stay
8 scenes per GPU), and sync_bn=True (GradNormLite EMA all-reduce; BN
convert is a no-op on PT-v3-malibu LayerNorm).
...
sync_bn = True  # required for DDP: syncs GradNormLite EMA across ranks
enable_amp = True

# Data parameters
# Train batch_size is the global effective batch (same as the 1-GPU recipe).
# val/test caps are N * num_gpu so per-GPU occupancy stays 8 after
# default_setup // world_size. Voxel budgets are already per-rank.
batch_size = 12  # total across GPUs (3 per GPU when num_gpu=4)
...
# Wandb parameters
wandb_run_name = (
    f"4xH100 ptv3 multi {grp_exp}.{num_exp} iters={total_iters} lr={lr} wd=5e-3"
)
...
# -----------------------------------------------------------------------------
# Dataset (grid_coord + voxel pipeline aligned with multi-spunet multitask)
# -----------------------------------------------------------------------------
