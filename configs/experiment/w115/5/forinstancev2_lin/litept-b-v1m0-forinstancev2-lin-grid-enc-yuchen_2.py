"""
LitePT-Base grid-search linear probing on ForInstanceV2 — Yuchen source only
(the single ULS-style site; `ForInstanceV2Dataset(sources=["Yuchen"])`),
encoder multiscale variant. Same frozen checkpoint (873542) and probe grid
as the sibling full-dataset config (`..._1.py`) in this folder -- see that
file for the shared rationale (no color/strength on disk, `in_channels=7`
with both faked via `FillMissingFeat`, etc.).

Yuchen has exactly 1 scene per split after preprocessing (its raw PLYs are
all under the preprocessing script's `--big_file_threshold`, so it's kept
as a single unvoxelized, unchunked scene each) -- by far the smallest of
the 3 setups in this folder. Every training step draws from that one scene;
diversity comes only from augmentation + random GridSample/SphereCrop
crops, not from distinct scenes.

Epoch budget: n_train=1 <= batch_size(24) -> `drop_last=False`
(`pointcept/engines/train.py`), so 1 step/epoch regardless of epoch count
(exactly the H3D regime: H3D's 15 train scenes are also < batch_size=24,
hence its epoch=2000 for 2000 total steps). To match the sibling
full-dataset/NIBIO-only configs' shared ~3000-total-step budget (chosen
between DALES's 4000 and H3D's 2000 -- both verified against actual saved
tile counts, not assumed from raw file counts, see `..._1.py`), epoch=3000
here too (3000 steps/epoch x 1 = 3000 total steps, same as NIBIO-only).
"""
...
wandb_run_name = (
    f"LitePT-B GridProbe ForInstanceV2 (Yuchen only) {grp_exp}.{num_exp}) encoder multiscale "
