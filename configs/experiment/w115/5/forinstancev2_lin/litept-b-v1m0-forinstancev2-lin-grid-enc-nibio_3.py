"""
LitePT-Base grid-search linear probing on ForInstanceV2 — NIBIO source only
(exact `source == "NIBIO"` match; the distinct `NIBIO2` and `NIBIO_MLS`
sources are a different acquisition/campaign from the same institute and
are NOT included here — edit `sources` below to merge them in if that's
what you actually want), encoder multiscale variant. Same frozen checkpoint
(873542) and probe grid as the sibling full-dataset config (`..._1.py`) in
this folder -- see that file for the shared rationale (no color/strength on
disk, `in_channels=7` with both faked via `FillMissingFeat`, etc.).

NIBIO has 8 train / 6 val / 6 test scenes after preprocessing (all raw
plot-sized files, never voxelized/chunked).

Epoch budget: n_train=8 <= batch_size(24) -> `drop_last=False`
(`pointcept/engines/train.py`), so 1 step/epoch regardless of epoch count
(same regime as H3D: its 15 train scenes are also < batch_size=24, hence
epoch=2000 for 2000 total steps there). To match the sibling full-dataset/
Yuchen-only configs' shared ~3000-total-step budget (chosen between
DALES's 4000 and H3D's 2000 -- both verified against actual saved tile
counts, not assumed from raw file counts, see `..._1.py`), epoch=3000
here too (3000 steps/epoch x 1 = 3000 total steps, same as Yuchen-only).
"""
...
wandb_run_name = (
    f"LitePT-B GridProbe ForInstanceV2 (NIBIO only) {grp_exp}.{num_exp}) encoder multiscale "
