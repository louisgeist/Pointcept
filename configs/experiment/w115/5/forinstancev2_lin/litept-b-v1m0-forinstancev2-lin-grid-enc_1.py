"""
LitePT-Base grid-search linear probing on ForInstanceV2 — full dataset,
encoder multiscale variant (same frozen checkpoint as the DALES/H3D
`-lin-grid-enc` siblings, job 873542, Flair3D+ multitask supervised
pretrain). `enc_mode=True` -> 1386ch concat of the 5 raw encoder stages
(54+108+216+432+576).

ForInstanceV2 has neither color nor intensity on disk (see
`pointcept/datasets/preprocessing/forinstancev2/preprocess_forinstancev2.py`)
-- both `color` and `strength` are zero-filled via `FillMissingFeat`, same
as H3D fakes `strength` (H3D does have real color, ForInstanceV2 has neither).
`in_channels=7` still matches the frozen backbone's trained input shape.

Epoch budget, derived by comparing against the other GridProbe datasets'
actual per-epoch step count (steps/epoch = n_train_scenes // batch_size when
`drop_last=True`, else ceil(n_train_scenes / batch_size); `drop_last =
n_train_scenes > batch_size` per `pointcept/engines/train.py`). Scene counts
verified against the actual saved tile folders, not assumed from raw file
counts (an earlier pass wrongly guessed DALES at 522 train scenes from
58 raw tiles x an assumed 3x3 chunking -- `ls data/dales/train` on Jean Zay
gives the real number, 261, and there's no separate DALES val split, `val`
reuses `test` per `configs/dales/*.py`):
  - DALES (261 train scenes, batch=24): 261>24 -> drop_last=True, 10 steps/epoch
    x epoch=400 -> 4000 total steps
  - H3D   (15 train scenes,  batch=24): 15<=24 -> drop_last=False, 1 step/epoch
    x epoch=2000 -> 2000 total steps
  - ForInstanceV2 full (121 train scenes, batch=24): 121>24 -> drop_last=True,
    5 steps/epoch. Target ~3000 total steps (between H3D's 2000 and DALES's
    4000) -> epoch=600 (600/5=3000 steps, 600/eval_epoch(10)=60 evals).
  This same ~3000-step target is used for the sibling Yuchen-only/NIBIO-only
  configs in this folder (1 step/epoch each -> epoch=3000) so all three
  setups get the *same* training budget and are directly comparable.
...
select_metric left at default (mIoU, matches DALES/ECLAIR/Flair3D
in-domain convention -- H3D is the one exception, using macro_f1).
skip_test=False: run the final full-test-set pass on the winning probe
(ForInstanceV2's test split is small/cheap, ~20s in an earlier smoke run).
"""
...
wandb_run_name = (
    f"LitePT-B GridProbe ForInstanceV2 (full) {grp_exp}.{num_exp}) encoder multiscale "
...
model = dict(
    ...
    backbone=dict(
        type="LitePT-v1",
        in_channels=7,  # coord(3) + color(3, fake) + strength(1, fake)
...
data = dict(
    ...
    train=dict(
        ...
        data_root=data_root,
        transform=[
            dict(type="FillMissingFeat", feat_key="color", feat_dim=3, fill_value=0.0),
