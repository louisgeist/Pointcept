...
litept-b-v1m0-forinstancev2-lin-grid-enc_1.py` — everything below is
identical to that config except `epoch` (600 -> 2400, quadrupling total
training steps from ~3000 to ~12000; `eval_epoch` left at 10, so eval
count also quadruples, 60 -> 240 evals). See the base config's docstring
for the full epoch-budget derivation and grid rationale.
