...
Variant _2: plain sum (GradNormLite disabled; each nathab axis w=0.25). 1× A100,
...
# Wandb parameters
wandb_run_name = (
    f"A100 LPT-B multi noGNL w111.2.{num_exp} iters={total_iters}"
)
...
del _network_ignore
# Without GradNormLite, give each nathab axis w=1/4 so the four axes jointly
# match one task slot (segment / forest / elevation / network).
task_weights = {task_name: 1.0 for task_name in task_configs.keys()}
task_weights.update({task_name: 0.25 for task_name in nathab_keys})
