"""
PT-v3-malibu grid-search linear probing on DALES — RANDOM-INIT backbone baseline.

Encoder multiscale variant (`enc_mode=True`, 992ch). Same probe grid and data
pipeline as configs/dales/ptv3-v1m0-dales-lin-grid-enc.py (pretrained Flair3D+
...
seed ensemble) to quantify downstream gain from Flair3D+ pretrain vs this
lower bound.
...
# No weight= — random-init backbone (see docstring). CheckpointLoader kept for phase 2.
...
wandb_run_name = (
    f"PT-v3-malibu GridProbe DALES RANDOM INIT H100 w111.2.{num_exp}) "
    f"enc {backbone_out_channels}ch, {len(probes)} probes, epoch={epoch}"
)
