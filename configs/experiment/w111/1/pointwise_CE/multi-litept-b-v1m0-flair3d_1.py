"""
LitePT-Base on Flair3D+ multitask (w111/1/pointwise_CE): segment (v20) +
forest_2d + elevation + 4 nathab tile_distribution axes trained with
point-wise CE + Lovasz (``pointwise_supervision``; Habitat Type / Moisture
Regime / Soil Chemistry / Bioclimatic Zone, remapped on the fly from
natural_habitat; inference still pools to tile distributions with KL/TV +
mIoU) + network (roads only, CE + foreground weight=5; railroads/transmission
lines dropped; scored via APLS at test time).

Standalone copy of configs/flair3d_default/multi-litept-b-v1m0-flair3d.py with
enable_nathab_pointwise_supervision enabled. Inherits only from default_runtime.
"""
...
# Wandb parameters
wandb_run_name = (
    f"LPT-B multi nathab-CE w111.{grp_exp}.{num_exp} iter={total_iters}"
)
