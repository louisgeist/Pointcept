"""
Sonata-v1m1 grid-search linear probing on DALES — w111/3/sonata_indoor (coord scale 1/5) — official indoor release baseline
...
backbone: in_channels=9 (coord+color+normal), stride=(2,2,2,2). DALES has no
RGB or normals — FillMissingFeat zero-fills both; intensity is not used (no
strength channel). Scene coords are rescaled by 1/5 before GridSample; grid_size=0.02 (native indoor Sonata pretrain).

AdamW / wd=0 / OneCycleLR warmup 5%, lr swept over 12 values. epoch=400 /
...
ignore_index = 8
grid_size = 0.02
coord_scale = 1 / 5  # aerial coord rescale; grid_size stays at native indoor 0.02
point_max = 102400
...
wandb_run_name = (
    f"Sonata-v1m1 indoor w111.3 GridProbe DALES {grp_exp}.{num_exp}) HF pretrain, "
