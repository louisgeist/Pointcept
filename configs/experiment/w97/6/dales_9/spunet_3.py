"""
SpUNet semantic segmentation on DALES (coord + strength LiDAR features).
...
coord_feat_scale = 1e-2
strength_feat_scale = 1.0
...
# Wandb parameters
wandb_run_name = (
    f"DALES SpUNet semseg ({grp_exp}.{num_exp}) "
    f"lr={lr} epoch={epoch} coord_scale={coord_feat_scale} "
    f"norm_str={normalize_strength} feat={','.join(feat_keys)}"
)
