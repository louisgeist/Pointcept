"""
LitePT-v1 mono-task natural_habitat on Flair3D+ with on-the-fly by_moisture_v3 remap.

On-disk labels must be preprocessed with --natural_habitat_definition default
(44 fine CarHab ids). Training applies Flair3DLabelRemap to by_moisture_v3 at load time.
(Cultivated, Built and Road are mapped to void.)
"""
...
# Wandb parameters
wandb_run_name = (
    f"Flair3D+ LitePT natural_habitat by_moisture_v3 {grp_exp}.{num_exp}) lr={lr}"
)
...
# On-disk NH labels use preprocess default (44 fine classes); train with by_moisture_v3.
label_definitions = dict(
    natural_habitat="by_moisture_v3",
)
