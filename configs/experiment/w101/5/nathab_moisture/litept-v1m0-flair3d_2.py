"""
LitePT-v1 mono-task natural_habitat on Flair3D+ with on-the-fly by_moisture_v2 remap.

On-disk labels must be preprocessed with --natural_habitat_definition default
(44 fine CarHab ids). Training applies Flair3DLabelRemap to by_moisture_v2 at load time.
(Cultivated, Built and Road are merged into Anthropogenic habitat.)
"""
...
# Wandb parameters
wandb_run_name = (
    f"Flair3D+ LitePT natural_habitat by_moisture_v2 {grp_exp}.{num_exp}) lr={lr}"
)
...
main_task = "natural_habitat"
target_keys = (main_task,)
origin_main_task = f"origin_{main_task}"

# On-disk NH labels use preprocess default (44 fine classes); train with by_moisture_v2.
label_definitions = dict(
    natural_habitat="by_moisture_v2",
)
storage_label_definitions = dict(
    natural_habitat="default",
)
natural_habitat_label_remap = dict(
    type="Flair3DLabelRemap",
    remaps=label_definitions,
    storage_definitions=storage_label_definitions,
)
