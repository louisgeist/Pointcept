...
wandb_run_name = f"KPConvX {exp_group}.{num_exp}) eff_bs={batch_size*gradient_accumulation_steps} | lr={lr} | grid_size={grid_size} | epoch={epoch} | warmup={warmup_steps} | scheduler=LinearLR | input=RGB only"
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            dict(type="ToTensor"),
            dict(type="Collect", keys=("coord", "segment",), feat_keys=["color"])
        ],
        test_mode=False,
    ),
