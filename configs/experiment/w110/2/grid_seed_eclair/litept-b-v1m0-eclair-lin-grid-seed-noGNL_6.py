...
epoch = 200
eval_epoch = 10
lr = 0.5  # fixed at the lr-grid sweep winner (see docstring)
patch_size = 1024
...
probes = {
    f"ce_lovasz_init{i}": dict(
        ...
        feat_norm=None,
        dropout=0.0,
        optimizer=dict(type="AdamW", lr=0.5, weight_decay=0.0),
        scheduler=dict(
            type="OneCycleLR",
            max_lr=0.5,
            pct_start=0.05,
...
wandb_run_name = (
    f"LitePT-B SeedEnsemble ECLAIR {grp_exp}.{num_exp}) enc noGNL job1288597, "
    f"AdamW/wd0/OneCycleLR warmup5%, {len(probes)} inits, lr=0.5, epoch={epoch}"
)
