...
Swapped from DALES to ECLAIR after the DALES attempt
(spunet-v1m0-dales-lin-grid-seed-debug_1.py) crashed spconv locally even with
ConvAlgo.Native forced: DALES's data.val/data.test have NO SphereCrop, so a
single whole DALES tile (~11M points post-GridSample, batch_size already 1 --
this isn't a batching issue) segfaults spconv on this machine regardless of
algo (same root cause already documented in
scripts/_grid_probe_extract_common.py's extraction-script docstring). ECLAIR
tiles are far smaller (raw coord.npy sizes locally range ~2.6k-800k points,
vs multi-million for DALES/H3D), so the same whole-tile-no-crop val/test path
should stay well inside what a single spconv forward can handle here.

Derived from configs/experiment/w110/2/grid_seed_eclair/spunet-v1m0-eclair-lin-grid-seed_5.py
with everything shrunk for speed:
  ...
  - SpconvNativeConvAlgo hook (local-only, see pointcept/engines/hooks/misc.py):
    hecate's spconv autotuner crashes on SpUNet's 2nd distinct conv shape.
  - data.{train,val,test}.max_sample=2 -- a couple of ECLAIR tiles instead of
    the full split.
  ...
  - wandb_project has a "_debug" suffix so this doesn't mix into the real
    pointcept_eclair project history.
"""
...
epoch = 4
eval_epoch = 2
lr = 0.01  # same fixed lr as the real spunet-v1m0-eclair-lin-grid-seed_5.py
...
# Hooks
# No CheckpointLoader: fresh random-init backbone (see docstring).
# SpconvNativeConvAlgo: local-only, hecate's spconv autotuner crashes on
# SpUNet's 2nd distinct conv shape -- not needed on JZ, not present in the
# real (non-debug) spunet-*-lin-grid-seed_*.py configs. Order otherwise
# matches every other seed-ensemble config: GridProbeEvaluator before
# GridProbeCheckpointSaver/CheckpointSaver; GridProbeSeedEnsembleTester last.
hooks = [
...
wandb_run_name = (
    f"[DEBUG] SpUNet SeedEnsemble ECLAIR {grp_exp}.{num_exp}) RANDOM INIT, "
