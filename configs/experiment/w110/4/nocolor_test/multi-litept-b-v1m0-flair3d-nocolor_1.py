"""
TEST-ONLY: Flair3D+ precise test of LitePT-Base multitask ckpt 873542 with RGB
always dropped (learned color fill-in), never the real color.

Standalone copy of configs/flair3d_default/multi-litept-b-v1m0-flair3d.py
(job 873542). Val and test apply RandomDropColor(drop_ratio=1.0,
drop_application_ratio=1.0, keep_mask=True) so MultiTaskSegmentorV2 replaces
every color channel with the checkpoint's learned color_mask_value. Strength
is unchanged. Do not train with this config.

Launch on Jean Zay via test_flair3d_resume.sh with CONFIG pointing here and a
fresh SAVE_PATH (do not write into logs/slurm/873542).
"""
...
# Frozen LitePT-B Flair3D+ multitask checkpoint (job 873542).
weight = "/lustre/fswork/projects/rech/unv/usi32yh/Pointcept/logs/slurm/873542/model/model_best.pth"

# Wandb parameters
wandb_run_name = (
    f"LPT-B multi {grp_exp}.{num_exp}) noRGB fill-in ckpt873542"
)
