...
RGB lr-grid winner as a starting point -- re-run grid_h3d_norgb first and refit lr
to its winner (or drive both phases via tools/grid_then_seeds.py).
...
wandb_run_name = (
    f"LitePT-B SeedEnsemble H3D noRGB {grp_exp}.{num_exp}) enc multiscale {backbone_out_channels}ch, "
