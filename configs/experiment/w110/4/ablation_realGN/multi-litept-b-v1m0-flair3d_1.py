"""
LitePT-Base on Flair3D+ multitask ablation, 4x H100: real GradNorm (Chen et al.
2018, "GradNorm: Gradient Normalization for Adaptive Loss Balancing in Deep
Multitask Networks") instead of the home-grown GradNormLite.

Same recipe as configs/experiment/w109/5/ablation/multi-litept-b-v1m0-flair3d_1.py
(itself the noGNL static-weight ablation of
flair3d_default/multi-litept-b-v1m0-flair3d.py) EXCEPT the multitask loss
balancer:
  - grad_norm_lite = False  ->  grad_norm = True
  - the 4 nathab axes are still pooled into one GradNorm group (grad_norm_task_groups,
    unchanged from grad_norm_lite_task_groups), so the task structure is identical
    to the GradNormLite run -- only the weighting algorithm changes.
  - task_weights back to a uniform 1.0 init (GradNorm learns the balance; no more
    the static 0.25-per-nathab-axis hack the noGNL config needed).

3-way comparison:
  GradNormLite : flair3d_default/multi-litept-b-v1m0-flair3d.py            (JZ 873542)
  static noGNL : configs/experiment/w109/5/ablation/multi-litept-b-...     (JZ 1288597 is lr5e-4 sibling)
  real GradNorm: this config

segment (v20) + forest_2d + elevation + 4 nathab tile_distribution axes +
network (roads only). Standalone; inherits only from default_runtime.
"""
...
# Real GradNorm (Chen et al. 2018). Mutually exclusive with grad_norm_lite.
grad_norm = True
grad_norm_alpha = 1.5        # restoring-force / asymmetry exponent (paper default)
grad_norm_weight_lr = 1e-2   # Adam lr for the learnable per-group loss weights
grad_norm_interval = 1       # update the weights every optimizer step (faithful)
...
# Wandb parameters
wandb_run_name = (
    f"4xH100 LPT-B multi realGradNorm a{grad_norm_alpha} "
    f"{grp_exp}.{num_exp}) iter={total_iters}"
)
...
# GradNorm: pool the 4 nathab axes into one "nathab" group (same mapping the
# GradNormLite run used as grad_norm_lite_task_groups). One learnable weight
# w_nathab, the 4 axis losses summed before the shared-layer gradient probe.
grad_norm_task_groups = {task_name: "nathab" for task_name in nathab_keys}
...
del _network_ignore
# Uniform init: GradNorm learns the per-group balance (weights renormalized to
# sum to #groups each step). No static per-task hack here (contrast the noGNL
# ablation, which needed task_weights[nathab_axis] = 0.25).
