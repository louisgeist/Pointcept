...
Rerun of configs/experiment/w110/3/scratch_h3d/semseg-litept-b-v1m0-h3d_4.py
after the H3D RGB preprocessing fix (fixed //256 descale instead of the
broken per-tile stretch, see pointcept/datasets/preprocessing/h3d/preprocess_h3d.py
git history) -- hyperparameters and architecture are otherwise unchanged from
the original run.

Standalone experiment copy of configs/h3d/semseg-litept-b-v1m0-h3d.py
(scratch, DefaultSegmentorV2), with H100 worker / wandb naming.
epoch=4000, lr=5e-4 (eval_epoch=100), the winning sweep member from the
original w110/3/scratch_h3d grid (the other 3 members were dropped).
Backbone dims and optimization recipe mirror the LitePT-Base used in
configs/flair3d_default/multi-litept-b-v1m0-flair3d.py, adapted to H3D
single-task semseg (no multitask wiring) and trained from scratch (no
pretrained weight).
...
# Wandb parameters
wandb_run_name = f"H3D LitePT-B semseg from-scratch ({grp_exp}.{num_exp}) H100 lr={lr} epoch={epoch}"
