...
Frozen backbone: Flair3D+ multitask supervised pretrain with real GradNorm
(Chen et al. 2018) instead of GradNormLite (job 1468317, see
configs/experiment/w110/4/ablation_realGN/multi-litept-b-v1m0-flair3d_1.py).
Standalone copy of configs/h3d/litept-b-v1m0-h3d-lin_enc.py with only the
checkpoint swapped — 3-way GradNorm-scheme comparison on H3D alongside the
GradNormLite baseline (litept-b-v1m0-h3d-lin_enc_1.py, job 873542) and the
noGNL static-weight ablation (litept-b-v1m0-h3d-lin-grid-enc-noGNL_7.py, job
1288597); previously only probed on OpenGF/PureForest, not H3D.
...
weight = "/lustre/fswork/projects/rech/unv/usi32yh/Pointcept/logs/slurm/1468317/model/model_best.pth"
...
wandb_run_name = (
    f"LitePT-B GridProbe H3D {grp_exp}.{num_exp}) enc realGN job1468317, "
