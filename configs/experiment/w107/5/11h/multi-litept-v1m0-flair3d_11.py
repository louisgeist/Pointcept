"""
LitePT-Small on Flair3D+ multitask: combine the two promising axes from the
_4-_10 ablation (w107/4/13h) into one run.

_11: batch_size=20 (from _4) + GradNormLite pooled nathab (from _5).
grad_norm_lite_task_groups maps all 4 nathab_keys to a single "nathab" group:
their last-layer grad norm is probed once on the summed nathab loss, one
shared EMA scale is learned, and that single scale is applied to each of the
4 nathab losses (segment/forest/elevation still get their own independent
scale). Rationale for pooling: with 4 independent GradNormLite scales, each
nathab axis gets pushed to a "fair share" on its own, so the 4 axes together
out-weigh segment ~4:1; pooling targets a single fair share for nathab as a
whole. Combining with bs=20 restores the last known-good multitask batch size
(w101/7/moisture_multi) while keeping that GradNormLite fix.
