...
Series 2.3: CE only with class weight=[1.0, 5.0] (Background, Foreground).
Same data/schedule as 2.1.
...
# Wandb parameters
wandb_run_name = (
    f"Flair3D+ LitePT mono network roads+railroads "
    f"{grp_exp}.{num_exp} CE road_w=5 lr={lr} iters={total_iters} filtered+strat_val"
)
...
# CE only + class weight up-weighting Foreground (road/rail pixels) vs Background.
