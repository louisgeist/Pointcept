"""
LitePT-Small on Flair3D+ multitask: same as toward_bm/_1 (elevation in meters,
no ×0.01 target scale) with a fixed task_weight of 0.01 on elevation.

_2 (toward_bm): _1 + task_weights["elevation"]=0.01 to offset the ~×100 larger
SmoothL1 magnitude in meter space (approximate substitute for the old ×0.01
target scale). SmoothL1 beta=1.0 unchanged. GradNormLite still active on top.
...
stride=(3, 3, 3, 3), batch_size=20, elevation=meters w=0.01, num_gpu=1.
"""
...
wandb_run_name = (
    ...
    f"grad_norm_lite=pooled_nathab elevation=meters w=0.01"
)
