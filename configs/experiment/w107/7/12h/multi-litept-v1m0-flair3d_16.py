"""
LitePT-Base on Flair3D+ multitask: same stack as _14 (GradNormLite pooled
nathab, lr=1e-3, stride=(3, 3, 3, 3)) but with LitePT-B official dims.

_16: LitePT-B + batch_size=6 (conservative starting point; calibrate via
find_max_batch_size) + GradNormLite pooled nathab + stride=(3, 3, 3, 3).
...
stride=(3, 3, 3, 3), batch_size=6, lr=1e-3, num_gpu=1.
"""
...
# Data parameters
batch_size = 6 * num_gpu  # total batch size across all gpus (LitePT-B; calibrate)
# Cap scenes/batch; actual packing uses *_voxel_budget (w105/6/19h: 2M worked).
batch_size_val = 4 * num_gpu
batch_size_test = 4 * num_gpu
...
# Wandb parameters
wandb_run_name = (
    f"Flair3D+ LitePT-B multi + nathab_distribution "
...
model = dict(
    ...
    backbone=dict(
        ...
        stride=stride,
        # LitePT-Base official dims (deeper/wider than Small in _14).
