"""
LitePT semantic segmentation on FOR-instance V2 (coord-only, no color/strength
in the raw data). 3 classes: ground / low vegetation / tree.

To restrict to a single source (e.g. the "Yuchen" ULS-style site), override
`data.train.sources` / `data.val.sources` / `data.test.sources`, e.g.:
    --options data.train.sources='["Yuchen"]' data.val.sources='["Yuchen"]'
...
# Features -- no color/strength in raw ForInstanceV2 PLYs, geometry only
feat_keys = ["coord"]
...
# Wandb parameters
wandb_run_name = f"ForInstanceV2 LitePT semseg ({grp_exp}.{num_exp}) lr={lr}"
wandb_project = "pointcept_forinstancev2"
...
model = dict(
    ...
    backbone=dict(
        type="LitePT-v1",
        in_channels=3,  # coord only -- no color/strength available
