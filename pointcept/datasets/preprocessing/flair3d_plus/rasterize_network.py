...
Loads graphs exported by Flair3D-build ``export_network_graphs.py``, densifies
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Rasterize Flair3D-build exported network graphs into network.npy "
            "binary masks on Flair3D+ tiles."
        )
    )
    p.add_argument(
        "--data_root",
        type=str,
        required=True,
        help="Flair3D+ data root (contains train/val/test/...)",
    )
    p.add_argument(
        "--network_graphs_root",
        type=str,
        required=True,
        help="Root of exported *_graph.gpkg files (Flair3D-build network_graphs)",
    )
    p.add_argument(
        "--split_manifest_csv",
        type=str,
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Flair3DDataset missing tiles. Default: "
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="ROI-level process workers (1 = sequential; try 12 on hecate, 24 on JZ)",
    )
    return p
