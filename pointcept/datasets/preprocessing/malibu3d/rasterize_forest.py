"""Standalone backfill: write forest_2d.npy (1, H, W) masks per Malibu3D tile.
...
Example (local machine, D067)::

python pointcept/datasets/preprocessing/malibu3d/rasterize_forest.py \
    --data_root data/malibu3d \
    --source_dataset_root data/malibu3d/raw \
    --split_manifest_csv data/malibu3d/raw/scene_split_manifest_D067.csv \
    --pixel_m 0.5

Example (cluster, full manifest)::
...
try:
    ...
    from preprocess_malibu3d_v2 import build_modality_patch_path  # type: ignore
except ImportError:  # pragma: no cover
    from pointcept.datasets.preprocessing.malibu3d.network_label_utils import (
        parse_bool_flag,
    )
    ...
    from pointcept.datasets.preprocessing.malibu3d.preprocess_malibu3d_v2 import (
...
def process_patch(
    ...
    *,
    ...
    # per-point `forest` task (preprocess_malibu3d_v2.py). Without this, a
    ...
    # rasterio reads north-up (row 0 = north/top); the Malibu3D grid
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Rasterize the FOREST GeoTIFF into forest_2d.npy on Malibu3D tiles."
    )
    ...
    p.add_argument(
        ...
        "same layout as preprocess_malibu3d_v2's --dataset_root.",
    )
