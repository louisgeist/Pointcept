"""
Preprocessing script for Malibu3D / LidarHD-like ASCII PLY data.
...
try:
    ...
    from malibu3d_label_remap import SUPPORTED_LABEL_REMAPS, build_segment
...
def folder_to_id(file_dir: str, row_id: int = None, col_id: int = None) -> str:
    """Generate a Malibu3D-style id from the last 2 folder names."""
...
def main_process():
    ...
    parser.add_argument(
        ...
        help="Root directory containing Malibu3D/LidarHD tiles",
    )
    parser.add_argument(
        "--output_root",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Label definition (simple or fusion) from Malibu3D mappings",
    )
    parser.add_argument(
        "--save_strength",
        action="store_true",
    ...
    for split in splits:
        ...
        if len(split_tile_dirs) > 0:
            print(
                f"\n--- Split '{split}' ---\n"
                f"Using predefined Malibu3D split with {len(split_tile_dirs)} tile folders"
            )
        else:
            print(
                f"\n--- Split '{split}' ---\n"
