r"""
Malibu3D v2 — manifest-driven preprocessing from pre-labeled PLY files.
...
named definitions in ``malibu3d_label_remap.py`` (CLI: ``--segment_definition``,
...
The split manifest CSV (e.g. ``data/malibu3d/raw/scene_split_manifest.csv``)
...
Conventions (must match scripts/build_csv_manifest.py):
    ...
    PLY path = {ply_root}/LIDARHD/{dept_year}_LIDARHD/{roi}/
               {dept_year}_LIDARHD_{roi}_{scene_i_j}.plydata/malibu3d/raw/scene_split_manifest_D075.csv
...
python pointcept/datasets/preprocessing/malibu3d/preprocess_malibu3d_v2.py \
    --output_root data/malibu3d \
    --split_manifest_csv data/malibu3d/raw/scene_split_manifest.csv \
    --num_workers 24 \
    --multilabel-only
...
local machine:
python pointcept/datasets/preprocessing/malibu3d/preprocess_malibu3d_v2.py \
    --ply_root data/malibu3d_build/data/malibu3d_label_enhanced \
    --dataset_root data/malibu3d/raw \
    --output_root data/malibu3d \
...
from malibu3d_label_remap import (  # noqa: E402
...
def setup_file_logger(log_file_path: str) -> logging.Logger:
    ...
    logger = logging.getLogger("preprocess_malibu3d")
...
def main_process():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional path to write preprocessing logs. Defaults to <output_root>/preprocess_malibu3d.log",
    )
    parser.add_argument(
        "--missing_scenes_file",
        type=str,
    ...
    for task_key in ("segment", "forest", "land_use", "natural_habitat"):
        parser.add_argument(
            ...
            help=(
                ...
                f"(see malibu3d_label_remap.LABEL_DEFINITIONS)."
            ),
        )
    ...
    log_file_path = args.log_file or os.path.join(args.output_root, "preprocess_malibu3d.log")
