"""
Malibu3D+ unified label definitions and LUT remapping. (LUT: look-up table)

Single registry for all semantic tasks (segment, forest, land_use, natural_habitat).
Used by preprocess_malibu3d_v2 and malibu3d_config_utils (training metadata).
...
# Malibu3D-build label=v17 (finer13): building split by land use, soil under vegetation.
...
# Malibu3D-build label=v18 (finer14): unknown building usage merged into void.
...
# Malibu3D-build label=v19/v20 (finer12): single Building class, no land-use split.
...
def _build_segment_v17_lut(num_raw: int = 256, void_train_id: int = 18) -> np.ndarray:
  """Identity for 0..18 (Malibu3D-build finer13); overflow maps to void."""
  lut = np.full(num_raw, void_train_id, dtype=np.int32)
  lut[: void_train_id + 1] = np.arange(void_train_id + 1, dtype=np.int32)
  return lut
...
def _build_segment_v18_lut(num_raw: int = 256, void_train_id: int = 17) -> np.ndarray:
  """Identity for 0..17 (Malibu3D-build finer14); overflow maps to void."""
  lut = np.full(num_raw, void_train_id, dtype=np.int32)
  lut[: void_train_id + 1] = np.arange(void_train_id + 1, dtype=np.int32)
  return lut


def _build_segment_v19_lut(num_raw: int = 256, void_train_id: int = 15) -> np.ndarray:
  """Identity for 0..15 (Malibu3D-build finer12); overflow maps to void."""
  lut = np.full(num_raw, void_train_id, dtype=np.int32)
  lut[: void_train_id + 1] = np.arange(void_train_id + 1, dtype=np.int32)
  return lut
...
def _register_segment_definitions() -> Dict[str, LabelDefinition]:
  ...
  segment_v19_lut = _build_segment_v19_lut()
  segment_v19_void = 15
  v19 = _make_definition(
    "segment",
    "v19",
  ...
  # Same LUT/names as v19; name tracks Malibu3D-build label=v20 for meta.json cleanliness.
...
def definition_to_task_config(definition: LabelDefinition) -> Dict[str, object]:
  """Convert a LabelDefinition to a Malibu3D semantic task config dict."""
