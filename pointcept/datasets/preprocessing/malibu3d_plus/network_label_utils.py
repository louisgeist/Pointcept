"""Load and write Malibu3D-build network graphs for Pointcept preprocessing.
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as f:
        ...
        if missing:
            raise ValueError(
                ...
                "Enrich it via Malibu3D-build export_network_graphs.py "
                "(split_manifest_csv=...)."
            )
        for row in reader:
...
class LoadedNetworkGraph:
    ...
    topology written by Malibu3D-build's ``write_pixel_graph_gpkg`` -- this IS the
