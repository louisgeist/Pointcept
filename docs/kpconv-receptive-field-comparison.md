# Comparaison KPConv / KPConvX — ScanNet vs DALES / Flair3D (ALS)

Notes pour adapter KPConvX à l’ALS (Flair3D+). `dl` = taille de cellule du premier sub-échantillonnage, en mètres (`first_subsampling_dl` chez KPConv ; `grid_size` / `subsample_size` chez KPConvX).
...
- KPConvX Flair3D : `configs/flair3d_default/multi-kpconvx-v1m0-flair3d.py`

## Paramètres

| Paramètre | Nom code (KPConvX) | KPConv ScanNet | KPConvX ScanNet | KPConv DALES | KPConvX Flair3D |
|---|---|---|---|---|---|
...
| Paramètre | Nom code | KPConv ScanNet | KPConvX ScanNet | KPConv DALES | KPConvX Flair3D |
|---|---|---|---|---|---|
...
| Grandeur | Formule | KPConv ScanNet | KPConvX ScanNet | KPConv DALES | KPConvX Flair3D |
|---|---|---|---|---|---|
...
| Layer | KPConv ScanNet `dl=0.04` | KPConvX ScanNet `dl=0.02` | KPConv DALES `dl=0.25` | KPConvX Flair3D `dl=0.10` |
|---|---|---|---|---|
...
| `radius_scaling` | Homothétie entre étages : kernel, \(\sigma\), et grille de pooling. KPConv original = 2.0 ; KPConvX papier / ScanNet = 2.2 ; Flair3D default = 3.0. |

## Recette ALS (KPConv DALES vs KPConvX Flair3D actuel)

La recette DALES ne scale pas l’indoor 2 cm → 10 cm. Elle coarsifie **et** impose un contexte métrique :

1. `dl = 0.25 m` (vs 0.10 m KPConvX Flair3D).
2. Sphère **20 m**, pas un budget de points. À 40k pts et `dl=0.1 m`, le crop XY réel est plutôt ~5–15 m en canopée.
3. Deformable dès le 3ᵉ étage (`ρ=5`) : dernier kernel = **20 m** = `in_radius`. KPConvX Flair3D default : `kp_radius=3.2` partout (~26 m au fond). Ablation `_1` : `kp_radius=2.3` (~5.4 m).
